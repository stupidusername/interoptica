...
class DeliveryOrder extends \yii\db\ActiveRecord
{
	...
	public function afterSave($insert, $changedAttributes) {
		if ($insert) {
			$this->order->status = Delivery::orderStatusMap()[$this->delivery->deliveryStatus->status];
			$this->order->save();
		}
	...
	public function rules()
	{
		return [
			...
			[
				['order_id'],
				'unique',
				'when' => function() { return count(self::find()->andWhere(['order_id' => $this->order_id])->innerJoinWith('delivery')->all()); },
